...
class RotateFinger @Inject constructor() : ProcessingStep() {
    ...
    override fun run(originalImage: Mat, processedImage: Mat): Mat? {
        val middle = calcCenterPointOfMat(processedImage!!)
        val pointPair = generatePointPair(middle, 20)

        val p1Contour = calcPointOnContour(pointPair.first, processedImage)
        val p2Contour = calcPointOnContour(pointPair.second, processedImage)
        ...
        return rotateImageByDegree(correctionAngle, processedImage)
    }

    operator fun Point.minus(p: Point) = Point(this.x - p.x, this.y - p.y)

    private fun rotateImageByDegree(correctionAngle: Double, processedImage: Mat): Mat {
        val rotMat: Mat
        val destination = Mat(processedImage.rows(), processedImage.cols(), processedImage.type())
        ...
        Imgproc.warpAffine(processedImage, destination, rotMat, destination.size())
    ...
    private fun calcPointOnContour(first: Point, processedImage: Mat): Point {
        val height = processedImage.cols()
        var pointOnContour = Point()

        for (i in 0 until height) {
            val pixel = processedImage.get(first.x.toInt(), first.y.toInt() + i)
